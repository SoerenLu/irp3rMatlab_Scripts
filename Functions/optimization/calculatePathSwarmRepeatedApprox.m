function sliderPos = calculatePathSwarmRepeatedApprox( robot , qStart, qTarget, pointArray, energyType)
...
dRange = 1; % dRange is the side length of the cube defining the search space
fun = @(x)objectiveFunction(x, robot, qStart, qTarget, bestTrajArray, energyType, true, parameterStruct );
for i=1:PSOrepeated_numOfPSOs
    % Passing range. The edgle length of the rectangle defining the search
    % space for PSO.
    dRange = dRange / 5; % the search-sapce-cubes size is decreased
    disp([' # first PSO repetition: new search space is a cube of side-length ', num2str(dRange)]);
    for j=1:nWaypoint
        range(j) = {[t_min(j)-dRange/2, t_min(j)+dRange/2]};
    end
    [t_min, minval] = particleSwarmOptimization(fun,nWaypoint,PSO_swarmsize,PSO_maxIterations, PSO_maxIterationsWithoutImprovement, range);
end
