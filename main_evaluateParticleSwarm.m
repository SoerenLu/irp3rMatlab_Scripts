...
%temporarily as a function
function [] = main_evaluateParticleSwarm(alphaParameter, betaParameter, swarmsizeParameter)
...
PSO_alpha = alphaParameter;   %weight of random movement
PSO_beta = betaParameter;    %weight of movement to current optimum
PSO_swarmsize = swarmsizeParameter;
...
if(useRestingLengths)
    ...
    while i<=10
        ...
        objFunSum = objFunSum + objectiveSwarm;
        objFunAvg = objFunSum/i;
    ...
    %% Save the averaged objfun in the dataTable
    dataTable = {[], 'Average PSO', objFunAvg, [], [],[],[]};
    dataTable = [oldDataTable; dataTable];
    
    %% Save dataTable in an Excel Spreadsheet
    filename = ['output/evaluate',globalOptimMethod,'_autogenerated_',dateString,'.xlsx'];
    ...
    %% Save Plots
        ...
        %safe
        savefig(['output/_SelfmotionJointSpace_'...
            'PathNo', num2str(pathNo),'_' ,dateString]);
        close all;
    
        %Plot 2 - Cartesian Space
            Plotter.animateRobotStatic (robot3R , 1);
        print(['output/_SelfmotionCartesianSpace_'...
            'PathNo', num2str(pathNo),'_' ,dateString],...
            '-dpng');
        close all;
...
%% Safe Parameters
     dataTable = {...
            ...
            [],'Maximum Iterations Without Improvement',PSO_maxIterationsWithoutImprovement,[],[]
            };
